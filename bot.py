...
CRYPTO_RATE_FILE = "crypto_rate.json"
CRYPTO_USED_TX_FILE = "crypto_used_txs.json"
DEFAULT_CRYPTO_RATE = 101.0
CRYPTO_WALLET_ADDRESS = "TBUNjNEeb3fYyLcjXSJ9yhwzB2fy1UaoC2"
BINANCE_ID_NUMBER = "1130156772"
USDT_TRC20_CONTRACT = "TXLAQ63Xg1NAzckPwKHvzw7CSEmLMEqcdj"
CRYPTO_PAYMENT_TIMEOUT_SECONDS = 1800
CRYPTO_AMOUNT_TOLERANCE = 0.01
...
def get_crypto_rate():
    try:
        with open(CRYPTO_RATE_FILE, "r") as f:
            data = json.load(f)
            rate = float(data.get("rate", DEFAULT_CRYPTO_RATE))
            if rate > 0:
                return rate
    except Exception:
        pass
...
def save_crypto_rate(rate):
    with open(CRYPTO_RATE_FILE, "w") as f:
        json.dump({"rate": float(rate)}, f)


def crypto_amount_text(cart):
    inr_price = float(cart["final_price"])
    rate = get_crypto_rate()
    usdt = round(inr_price / rate, 2)
    return int(inr_price) if inr_price.is_integer() else inr_price, rate, usdt


def load_used_crypto_txs():
    try:
        with open(CRYPTO_USED_TX_FILE, "r") as f:
            data = json.load(f)
...
def save_used_crypto_txs(tx_ids):
    try:
        with open(CRYPTO_USED_TX_FILE, "w") as f:
            json.dump(list(tx_ids)[-1000:], f)
    except Exception:
        pass


def get_recent_usdt_trc20_payments():
    url = (
        "https://apilist.tronscanapi.com/api/token_trc20/transfers"
        f"?limit=50&start=0&sort=-timestamp&count=true"
        f"&relatedAddress={CRYPTO_WALLET_ADDRESS}"
    ...
    for tx in data.get("token_transfers", []):
        to_addr = tx.get("to_address") or tx.get("transferToAddress") or tx.get("toAddress")
        tx_id = tx.get("transaction_id") or tx.get("transactionHash") or tx.get("hash")
        ts = tx.get("block_ts") or tx.get("timestamp") or 0

        if to_addr != CRYPTO_WALLET_ADDRESS or not tx_id:
            continue

        token_info = tx.get("tokenInfo") or tx.get("token_info") or {}
        decimals = int(token_info.get("tokenDecimal") or token_info.get("decimals") or 6)
        raw_amount = tx.get("quant") or tx.get("amount") or tx.get("value") or 0
        ...
        payments.append({"tx_id": tx_id, "amount": amount, "timestamp": int(ts)})

    return payments


def find_matching_crypto_payment(expected_amount, created_at):
    used_txs = load_used_crypto_txs()
    min_ts = int((created_at - 120) * 1000)

    for payment in get_recent_usdt_trc20_payments():
        if payment["tx_id"] in used_txs:
            continue

        if payment["timestamp"] and payment["timestamp"] < min_ts:
            continue

        if abs(float(payment["amount"]) - float(expected_amount)) <= CRYPTO_AMOUNT_TOLERANCE:
            used_txs.add(payment["tx_id"])
            save_used_crypto_txs(used_txs)
            return payment
...
async def payinfo(update, context):
    ...
    kb = [
        ...
        [InlineKeyboardButton("💳 Payment Via Crypto", callback_data="crypto_payment")],
...
# ================= CRYPTO PAYMENT =================
async def crypto_payment(update, context):
    q = update.callback_query
    await q.answer()

    kb = [
        [InlineKeyboardButton("Pay Via Binance ID 🪪", callback_data="binance_id")],
        [InlineKeyboardButton("Pay via Crypto wallet Address", callback_data="crypto_wallet")]
    ]
...
async def binance_id(update, context):
    q = update.callback_query
    await q.answer()
    ...
    _, _, usdt = crypto_amount_text(cart)

    await q.message.reply_text(
        f"🪪 <b>Binance ID</b>\n\n"
        f"<code>{BINANCE_ID_NUMBER}</code>\n\n"
        f"💰 <b>Pay Amount: {usdt} USDT</b>\n\n"
        f"⚠️ Binance ID payment manual approval se verify hoga. Payment ke baad screenshot bheje.",
        parse_mode="HTML"
    )
...
async def crypto_wallet(update, context):
    ...
    CRYPTO_PENDING_PAYMENTS[q.from_user.id] = {
        "cart": copy.deepcopy(cart),
        "amount": usdt,
        "created_at": now_ts,
        "expires_at": now_ts + CRYPTO_PAYMENT_TIMEOUT_SECONDS
    }

    kb = [
        [InlineKeyboardButton("🔄 Check Payment", callback_data="check_crypto_payment")],
        [InlineKeyboardButton("📞 Contact Admin", url=f"https://t.me/{ADMIN_USERNAME}")]
    ]

    await q.message.reply_text(
        f"💳 <b>Crypto Wallet Address</b>\n\n"
        f"TRX / USDT (TRC20)\n"
        f"<code>{CRYPTO_WALLET_ADDRESS}</code>\n\n"
        f"💰 <b>Pay Amount: {usdt} USDT</b>\n\n"
        f"✅ Exact amount bhejne ke baad payment auto verify ho jayega.",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode="HTML"
    )


async def check_crypto_payment(update, context):
    q = update.callback_query
    await q.answer()

    pending = CRYPTO_PENDING_PAYMENTS.get(q.from_user.id)
    if not pending:
        await q.message.reply_text("❌ No pending crypto payment found. Please select wallet payment again.")
        return

    if datetime.now().timestamp() > pending["expires_at"]:
        CRYPTO_PENDING_PAYMENTS.pop(q.from_user.id, None)
        await q.message.reply_text("❌ Crypto payment rejected. Correct payment not found in time.")
        return

    payment = find_matching_crypto_payment(pending["amount"], pending["created_at"])
    if not payment:
        await q.message.reply_text("⏳ Payment not found yet. Agar amount galat hai to auto approve nahi hoga.")
        return

    await approve_crypto_payment(context.bot, q.from_user.id, pending["cart"], payment)
    CRYPTO_PENDING_PAYMENTS.pop(q.from_user.id, None)
...
# ================= APPROVE =================
async def grant_access_after_payment(bot, uid, cart):
    now = datetime.now()
    ...
    return username, purchase_date, expiry_date


async def approve_crypto_payment(bot, uid, cart, payment):
    username, purchase_date, expiry_date = await grant_access_after_payment(bot, uid, cart)
    ...
    await notify_admins(
        ...
        admin_detail_text(
            ...
            title="✅ CRYPTO PAYMENT AUTO APPROVED",
            item_type=cart["item_type"],
            base_price=cart["base_price"],
            combo_discount=cart["combo_discount"],
            coupon_code=cart["coupon_code"],
...
async def approve(update, context):
    ...
    username, purchase_date, expiry_date = await grant_access_after_payment(context.bot, uid, cart)
...
async def admin(update, context):
    ...
    kb = [
        ...
        [InlineKeyboardButton("💱 Set Crypto Rate", callback_data="set_crypto_rate")]
    ]
...
async def set_crypto_rate(update, context):
    q = update.callback_query
    await q.answer()

    reset_admin_flags(context)
    context.user_data["set_crypto_rate"] = True

    await q.message.reply_text(
        f"💱 Current Crypto Rate: ₹{get_crypto_rate():g} / USDT\n\n"
        f"Send new USDT rate. Example: 101"
    )
...
async def handle_text(update, context):
    ...
    try:
        # SET CRYPTO RATE
        if context.user_data.get("set_crypto_rate"):
            rate = float(txt.strip())
            if rate <= 0:
                await update.message.reply_text("❌ Rate 0 se bada hona chahiye.")
                reset_admin_flags(context)
                return

            save_crypto_rate(rate)
            await update.message.reply_text(f"✅ Crypto Rate Set: ₹{rate:g} / USDT")
            reset_admin_flags(context)
            return
...
# ================= CRYPTO AUTO VERIFY JOB =================
async def crypto_auto_verify_job(context):
    now_ts = datetime.now().timestamp()

    for uid, pending in list(CRYPTO_PENDING_PAYMENTS.items()):
        if now_ts > pending["expires_at"]:
            CRYPTO_PENDING_PAYMENTS.pop(uid, None)
            await safe_send(
                context.bot,
                uid,
                "❌ Crypto payment rejected. Correct payment not found in time."
            )
            await notify_admins(
                context.bot,
                f"❌ CRYPTO PAYMENT AUTO REJECTED\n\n"
                f"🆔 User ID: {uid}\n"
                f"💰 Expected Amount: {pending['amount']} USDT\n"
                f"📝 Reason: Correct payment not found in time."
            )
            continue

        payment = find_matching_crypto_payment(pending["amount"], pending["created_at"])
        if payment:
            await approve_crypto_payment(context.bot, uid, pending["cart"], payment)
            CRYPTO_PENDING_PAYMENTS.pop(uid, None)
...
app.add_handler(CallbackQueryHandler(crypto_payment, pattern="^crypto_payment$"))
app.add_handler(CallbackQueryHandler(binance_id, pattern="^binance_id$"))
app.add_handler(CallbackQueryHandler(crypto_wallet, pattern="^crypto_wallet$"))
app.add_handler(CallbackQueryHandler(check_crypto_payment, pattern="^check_crypto_payment$"))
...
app.add_handler(CallbackQueryHandler(set_crypto_rate, pattern="^set_crypto_rate$"))
...
app.job_queue.run_repeating(crypto_auto_verify_job, interval=60, first=20)
