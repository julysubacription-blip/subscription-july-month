...
async def binance_id(update, context):
    ...
    inr_price, rate, usdt = crypto_amount_text(cart)

    await q.message.reply_text(
        f"🪪 <b>Binance ID</b>\n\n"
        f"<code>1130156772</code>\n\n"
        f"💰 <b>Crypto Pay Amount</b>\n"
        f"<b>₹{inr_price} ÷ ₹{rate:g} = {usdt} USDT</b>\n"
        f"Rate: ₹{rate:g} / USDT",
        parse_mode="HTML"
    )
...
async def crypto_wallet(update, context):
    ...
    await q.message.reply_text(
        ...
        f"Trx (Trc20)\n"
        f"<code>TBUNjNEeb3fYyLcjXSJ9yhwzB2fy1UaoC2</code>\n\n"
        f"💰 <b>Crypto Pay Amount</b>\n"
        f"<b>₹{inr_price} ÷ ₹{rate:g} = {usdt} USDT</b>\n"
        f"Rate: ₹{rate:g} / USDT",
        parse_mode="HTML"
